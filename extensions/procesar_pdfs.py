...
contador_numero = 0
def procesar_pdfs_en_carpeta_para_post(carpeta, url_post):
...
def calcular_semana_de_trabajo(fecha):
    # Definir el inicio de la primera semana de trabajo
    inicio_semana = datetime(fecha.year, fecha.month, 16)
    
    # Si la fecha es antes del 16, retroceder un mes
    if fecha.day < 16:
        inicio_semana = inicio_semana - timedelta(days=fecha.day + 1)
        inicio_semana = datetime(inicio_semana.year, inicio_semana.month, 16)
    
    # Calcular la diferencia en días entre la fecha y el inicio de la primera semana
    diferencia_dias = (fecha - inicio_semana).days
    
    # Calcular la semana de trabajo
    semana_de_trabajo = (diferencia_dias // 7) + 1
    
    return semana_de_trabajo
